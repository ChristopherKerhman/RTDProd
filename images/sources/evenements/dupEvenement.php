...
$dataTraiter = $Evenement->getOneEvenement($idEvenement, $_SESSION['idUser']);
 ?>
 <h3 class="titrePage">Dupliquer cette séance ?</h3>
   ...
   <select id="public" name="public">
     <?php
       for ($i=0; $i < count($public) ; $i++) {
         if ($dataTraiter[0]['public'] == $i) {
           echo '<option value="'.$i.'" selected>'.$public[$i].'</option>';
         } else {
 ...
 <button type="submit" class="buttonForm">Dupliquer séance</button>
 </form>
